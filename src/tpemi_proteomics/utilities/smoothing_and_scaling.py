...
def one_sample_ttest(compiled, sample_cols, popmean=1):
    df = compiled.copy()
    ttest_results = []
    for (sequence, sample_name), df in df.groupby(['Sequence', 'sample_name']):
        results = []
...
def pval_smoothing(compiled, sample_cols, center=1):
    """Record point as mean * 1/p-val"""
    ttest_results = one_sample_ttest(compiled, sample_cols, popmean=center)
    ttest_results['exp_p-val'] = - 20**ttest_results['p-val']
    p_vals = pd.pivot_table(ttest_results, values='exp_p-val', index=['Sequence', 'sample_name'], columns='channel')

    proportional_pval = compiled.groupby(['Sequence', 'sample_name']).mean().copy()
    proportional_pval[sample_cols] = (center - proportional_pval[sample_cols]) * (1 / p_vals)
...
def channel_labels(input_path):
    raw_channels = pd.read_excel(f'{input_path}', sheet_name=None)
    channels = []
    for sample_name, channel_info in raw_channels.items():
        if sample_name != 'Description':
            channel_info['sample_name'] = sample_name
            channels.append(channel_info)
    channels = pd.concat(channels)
    channels = pd.melt(
        channels, 
        id_vars=['Channel', 'Label', 'sample_name'], 
        value_vars=[col for col in channels if 'Replicate' in col],
        var_name=['replicate'], 
        value_name='channel_contents',)
    channels['replicate'] = channels['replicate'].str.strip('Replicate ').astype(int)
    channels.columns = ['channel', 'label', 'sample_name', 'replicate', 'channel_contents']
    channels.dropna(subset=['channel_contents'], inplace=True)
    channels['key'] = channels["sample_name"].str.cat(
        channels[["replicate", "channel"]].astype(str), sep="_")
    return dict(channels[['key', 'channel_contents']].values)
